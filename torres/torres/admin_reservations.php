...
$reservations = $conn->query("SELECT r.*, u.email, u.program, u.year_level FROM reservations r LEFT JOIN users u ON r.student_id = u.id_number ORDER BY r.created_at DESC");
...
if ($filter == 'pending') {
    $reservations = $conn->query("SELECT r.*, u.email, u.program, u.year_level FROM reservations r LEFT JOIN users u ON r.student_id = u.id_number WHERE r.status = 'pending' ORDER BY r.created_at DESC");
} elseif ($filter == 'approved') {
    $reservations = $conn->query("SELECT r.*, u.email, u.program, u.year_level FROM reservations r LEFT JOIN users u ON r.student_id = u.id_number WHERE r.status = 'approved' ORDER BY r.created_at DESC");
} elseif ($filter == 'cancelled') {
    $reservations = $conn->query("SELECT r.*, u.email, u.program, u.year_level FROM reservations r LEFT JOIN users u ON r.student_id = u.id_number WHERE r.status = 'cancelled' ORDER BY r.created_at DESC");
}
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <nav class="bg-gradient-to-r from-blue-800 to-purple-800 text-white shadow-lg">
        <div class="container mx-auto px-4 py-3 flex justify-between items-center">
            ...
            <div class="flex items-center gap-4">
                ...
                <a href="admin_dashboard.php?tab=current_sit_in" class="hover:text-yellow-300 transition"><i class="fa-solid fa-clock mr-1"></i>Current Sit-in</a>
        ...
        <div class="bg-white rounded-xl shadow-lg mb-6">
            <div class="border-b border-gray-200">
                <nav class="flex -mb-px">
                    <a href="?filter=all" class="py-4 px-6 border-b-2 font-medium text-sm <?php echo $filter == 'all' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'; ?>">All</a>
                    <a href="?filter=pending" class="py-4 px-6 border-b-2 font-medium text-sm <?php echo $filter == 'pending' ? 'border-yellow-500 text-yellow-600' : 'border-transparent text-gray-500 hover:text-gray-700'; ?>">Pending <?php if ($pending_reservations > 0): ?><span class="ml-2 bg-yellow-100 text-yellow-800 py-0.5 px-2 rounded-full text-xs"><?php echo $pending_reservations; ?></span><?php endif; ?></a>
                    <a href="?filter=approved" class="py-4 px-6 border-b-2 font-medium text-sm <?php echo $filter == 'approved' ? 'border-green-500 text-green-600' : 'border-transparent text-gray-500 hover:text-gray-700'; ?>">Approved</a>
                    <a href="?filter=cancelled" class="py-4 px-6 border-b-2 font-medium text-sm <?php echo $filter == 'cancelled' ? 'border-red-500 text-red-600' : 'border-transparent text-gray-500 hover:text-gray-700'; ?>">Denied</a>
                </nav>
            </div>
        </div>
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">
                                <span class="px-2 py-1 text-xs rounded-full bg-purple-100 text-purple-800">Lab <?php echo $res['lab_room']; ?></span>
                            ...
                            <td class="px-6 py-4">
                                <!-- ✅ FIXED: Use start_time and end_time -->
                                <p class="text-sm font-semibold"><?php echo date('g:i A', strtotime($res['start_time'])); ?> - <?php echo date('g:i A', strtotime($res['end_time'])); ?></p>
                            </td>
                            <td class="px-6 py-4"><?php echo $res['duration']; ?> hour(s)</td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                ...
                                <div class="flex gap-2">
                                    <button onclick="openModal(<?php echo $res['id']; ?>, '<?php echo htmlspecialchars($res['student_name']); ?>', 'approve')" class="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded-lg text-sm transition">
                                        <i class="fa-solid fa-check mr-1"></i>Approve
                                    </button>
                                    <button onclick="openModal(<?php echo $res['id']; ?>, '<?php echo htmlspecialchars($res['student_name']); ?>', 'deny')" class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded-lg text-sm transition">
                                        <i class="fa-solid fa-times mr-1"></i>Deny
                                    </button>
                                </div>
                                <?php else: ?>
                                <span class="text-sm <?php echo $res['status'] == 'approved' ? 'text-green-600' : 'text-red-600'; ?>">
                                    <?php echo ucfirst($res['status']); ?>
                                </span>
                                <?php endif; ?>
                            </td>
        ...
        <div class="bg-white rounded-xl shadow-2xl p-8 max-w-md w-full mx-4">
            ...
            <form method="POST" class="space-y-4">
                ...
                <div class="bg-gray-50 p-4 rounded-xl">
                    <p class="text-gray-700"><span id="modal_student_name" class="font-bold"></span></p>
                ...
                <div>
                    ...
                    <textarea name="admin_notes" placeholder="Add a note..." rows="3" class="w-full px-4 py-2 border rounded-lg focus:border-purple-500 focus:outline-none"></textarea>
                </div>
                
                <div class="flex gap-4 pt-4">
                    <button type="button" onclick="closeModal()" class="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-800 font-semibold py-3 rounded-xl transition">Cancel</button>
                    <button type="submit" id="modal_submit_btn" class="flex-1 font-semibold py-3 rounded-xl transition"></button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        function openModal(id, studentName, action) {
            ...
            if (action === 'approve') {
                ...
                document.getElementById('modal_action_text').textContent = 'This will approve the reservation and add student to Current Sit-in.';
                ...
                document.getElementById('modal_action_text').textContent = 'This will deny the reservation.';
